...
export default function AdminInsightPage() {
  ...
  function openDeleteModal(insight) {
    if (!isAdmin) {
      setErrorMessage(
        "Hanya administrator yang dapat menghapus Insight."
      );
  ...
  async function handleDeleteInsight() {
    ...
    const insightToDelete = deleteTarget;
    ...
    try {
      ...
      setInsights((currentInsights) =>
        currentInsights.filter(
          (insight) =>
            insight.id !== insightToDelete.id
        )
      );

      const hasCover =
        Boolean(
          result?.deletedInsight?.cover_image_url
        ) ||
        Boolean(insightToDelete.cover_image_url);

      const coverDeleteFailed =
        hasCover &&
        result?.coverDeleted === false;

      setSuccessMessage(
        coverDeleteFailed
          ? `Insight “${insightToDelete.title}” berhasil dihapus dari database, tetapi file cover belum berhasil dihapus dari Storage.`
          : `Insight “${insightToDelete.title}” berhasil dihapus.`
      );
          ...
          <div className="flex flex-wrap items-center justify-between gap-5">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500">
                Kelola artikel, berita, dan informasi
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            ...
            {role === "editor" && (
              <span className="text-xs text-slate-400">
                ...
                tetapi tidak dapat menghapus Insight.
              </span>
            )}
          </div>
          ...
          <section className="mt-8 grid gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[minmax(0,1fr)_220px_auto]">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Cari judul, kategori, slug, atau penulis..."
            ...
            {loading ? (
              ...
              <div className="px-6 py-16 text-center">
                ...
                <p className="mt-2 text-sm text-slate-500">
                  {insights.length === 0
                    ? "Belum ada Insight di database."
                    : "Tidak ada Insight yang sesuai dengan pencarian atau filter."}
                </p>
                      ...
                      >
                        ...
                        <td className="px-6 py-5">
                          <div className="flex justify-end gap-2">
                            <Link
                              to={`/admin/insight/edit/${insight.id}`}
          ...
          <div className="w-full max-w-md rounded-3xl bg-white p-7 shadow-2xl">
            ...
            {deleteTarget.cover_image_url && (
              <p className="mt-3 rounded-xl bg-slate-50 px-4 py-3 text-xs leading-5 text-slate-500">
                File cover Insight juga akan dihapus dari
                Supabase Storage.
              </p>
            )}
